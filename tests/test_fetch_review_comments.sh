...
test_docs_cover_two_step_scope_flow() {
  ...
  grep -q 'npx skills check' "$ROOT_DIR/README.md"
  grep -q 'npx skills update' "$ROOT_DIR/README.md"
  grep -q 'skills/review-pr/README.md' "$ROOT_DIR/README.md"
  ! grep -qi 'Workflow Summary' "$ROOT_DIR/README.md"
  ! grep -qi 'Local Development Commands' "$ROOT_DIR/README.md"
  ! grep -qi 'resolved code-review feedback is excluded by default' "$ROOT_DIR/README.md"
  grep -q '^# Review PR$' "$SKILL_DIR/README.md"
  grep -q '```mermaid' "$SKILL_DIR/README.md"
  grep -qi 'ask the user whether to include code-review comments in scope' "$SKILL_DIR/README.md"
  grep -qi 'only the approved comment categories' "$SKILL_DIR/README.md"
  grep -qi 'resolved code-review feedback is excluded by default' "$SKILL_DIR/README.md"
  grep -qi 'outdated threads are validated separately from unresolved threads' "$SKILL_DIR/README.md"
  grep -qi 'search only within changed files for same-pattern candidates' "$SKILL_DIR/README.md"
